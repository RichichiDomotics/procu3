...
$(document).ready(function() {
    ...
    $("#fechaEnvio").val(getFechaActualMich(fechaActual)+" "+getHoraMinutoActualMich(fechaActual));
//    $("#horaSolicitud").html(getHoraMinutoActual(fechaActual));
    ...
    if (!perteneceAPoderJudicial) {
        ...
        ejecutaAction("/consultarDatosUsuario", function(usuario){
            ...
            var cveFuncionario = $(usuario).find("funcionario").find("claveFuncionario").first().text();
            $("#nombreDelSolicitante").val(nombre + " " + apellidoPaterno + " " + apellidoMaterno);
            $("#nombreDelSolicitante").attr("readonly", "true");
            $("#cveFuncionario").val(cveFuncionario);
            $("#cveFuncionario").attr("readonly", "true");
            $("#institucionSolicitante").val($(usuario).find("funcionario").find("departamento").last().find("nombreDepto").text());
            $("#unidadInv").val($(usuario).find("discriminante").find("catDiscriminanteId").last().text());
            $("#promovente").val(1);
            $("#materia").val("P");
        ...
        ejecutaAction("/consultarInvolucradoPorCalidadCaso", function(involucrados){
            ...
            $(involucrados).find('involucrado').each(function(){
                ...
                var fechaIni = $(this).find("expedienteDTO").find("fechaApertura").first().text();
//                $("#fechaInicio").html(fechaIni);
			...
			var prioridad = $(involucrados).find('involucrado').find("situacionImpDTO").find("prioridad").text();
			$("#prioridad").val(prioridad);
			$("#prioridad").attr("readonly", "true");
			
			var tipoInicio = $(involucrados).find('involucrado').find("esDetenido").text();
			if (tipoInicio == "true")
				$("#tipoIni").val(1);
			else
				$("#tipoIni").val(2);
...
function getFechaActualMich(fechaActual){
    var month = fechaActual.getMonth() + 1;
    var mes = month < 10 ? "0" + month: month;
