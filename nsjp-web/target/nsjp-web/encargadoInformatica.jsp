	...
	$(document).ready(function() {
		...
		*/
		function cargaGridSolicitudesDeAudioVideo(estatusSolicitado){
			...
			/*	
			 *	Enable IT : cambiando validacion para que el encargadoinf pueda ver las 
			 *	solicitudes del dia y pueda dar seguimiento 
			 *	,ya que solo consultaba las solciitudes en proceso y finalizadas 
			 **/
			if(estatusSolicitado == 'delDia'){
				estatus="<%=EstatusSolicitud.EN_PROCESO.getValorId()%>";
			}else	if(estatusSolicitado == 'cerrada'){
		...
		*/
		function dblClickRowBandejaAudiencias(solicitudId,solicitante){
			...
			$.newWindow({id:"iframewindowAtenderSolicitudDeAudioVideo"+idWindowVisorAudioVideo, statusBar: true, posx:111,posy:5,width:910,height:650,title:"Atender Solicitud de Audio/Video", type:"iframe"});
	    	$.updateWindowContent("iframewindowAtenderSolicitudDeAudioVideo"+idWindowVisorAudioVideo,'<iframe src="<%=request.getContextPath()%>/atenderSolicitudAudienciaInformatica.do?solicitudId='+solicitudId+'&solicitante='+solicitante+'" width="910" height="650" />'); 
		}
