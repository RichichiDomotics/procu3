...
<head>
	...
	//Variable que controla que se cargue por primera vez el grid de solicitudes de dictamen en proceso
	var firstGridSolicitudesDeDictamenCanceladas = true;
	...
	function restablecerPantallas(){
		...
		ajustarGridAlCentro($("#gridSolicitudesDeDictamenCanceladas"));
	...
	*/
	...
	,	center: {
		...
		,	onresize_end:			function () { $("#gridDetalleEvidencias").setGridWidth($("#mainContent").width() - 5, true);
										...
										$("#gridSolicitudesDeDictamenCanceladas").setGridWidth($("#mainContent").width() - 5, true);
	...
	*/
	function gridEvidencias(){
		...
		jQuery("#gridDetalleEvidencias").jqGrid({ 
			...
			datatype: "xml", 
			colNames:['Número','Número de Caso','Tipo de solicitud','Nombre del Solicitante','Cadena de Custodia','Perito Responsable','Fecha Límite','Acuse' ], 
	...
	function ejecutaChat() {
  	...
  	//Abre una nueva ventana para designar un perito /*Se comenta ByYolo Testeando*/		
...
// 	}
	...
	function designarPeritoTest(deshabilitarPestanas,deshabilitarBotonSP,numidExpedienteDoc,solicitudPericialId, estatus, area){
  	...
  	//Abre una nueva ventana para designar un perito/*ByYolo*/ 		
	function visorCanceladasPericiales(habilitarDetalleCancelacion,deshabilitarPestanas,deshabilitarBotonSP,numidExpedienteDoc,solicitudPericialId, estatus, area){
		idWindowDesignarPerito++;
		$.newWindow({id:"iframewindowConsultarDenunciante" + idWindowDesignarPerito, statusBar: true, posx:50,posy:50,width:1150,height:550,title:"Designar Perito", type:"iframe"});
		$.updateWindowContent("iframewindowConsultarDenunciante" + idWindowDesignarPerito,'<iframe src="<%= request.getContextPath() %>/designarPerito.do?solicitudPericialId=' +solicitudPericialId +'&estatus='+estatus+'&numidExpedienteDoc='+numidExpedienteDoc+'&area='+area+'&deshabilitarBotonSP='+deshabilitarBotonSP+'&deshabilitarPestanas='+deshabilitarPestanas+'&habilitarDetalleCancelacion='+habilitarDetalleCancelacion+'" width="1350" height="550" />');
	}
	...
	*/
	function ocultaMuestraGrids(nombreGrid){
		...
		$("#divGridSolicitudesDeDictamenCanceladas").hide();
		...
		switch (nombreGrid){
       		 case "gridSolicitudesDeDictamenCanceladas":
        		$("#divGridSolicitudesDeDictamenCanceladas").show();
            	break;
	...
	function cargaGridSolicitudesDeDictamenNoAtendidas(){
		...
		if(firstGridSolicitudesDeDictamenNoAtendidas == true){
			...
			jQuery("#gridSolicitudesDeDictamenNoAtendidas").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['Número de Folio','Número de Caso','Número','Nombre del Solicitante','Especialidad de Perito','Fecha de Solicitud' ], 
	...
	function cargaGridSolicitudesDeDictamenProceso(){
		...
		if(firstGridSolicitudesDeDictamenEnProceso == true){
			...
			jQuery("#gridSolicitudesDeDictamenEnProceso").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['Número de Folio','Número de Caso','Número','Nombre del Solicitante','Especialidad de Perito','Fecha de Solicitud','Fecha Última Modificación' ], 
				colModel:[ 	{name:'NumeroFolio',index:'1', width:200},
							{name:'NumeroCaso',index:'2', width:200},
				           	{name:'NumeroExpediente',index:'3', width:200},
				           	{name:'NombreSolicitante',index:'4', width:230},
				...
				ondblClickRow: function(rowid) {
					/*mandando variales/*ByYolo*/
 					...
 					var numidExpedienteDoc=ret2.NumeroExpediente;
 					designarPeritoTest("false","true",numidExpedienteDoc,rowid,"NA",1);
 					/*fin/*ByYolo*/
// 					designarPerito(rowid,"NA",1);
				},				
				sortorder: "desc"
	...
	/*
	*Funcion que carga el grid de solicitudes periciales Cancelada yolotzin
	*/
	function cargaGridSolicitudesDeDictamenCanceladas(){

		if(firstGridSolicitudesDeDictamenCanceladas == true){
			
			jQuery("#gridSolicitudesDeDictamenCanceladas").jqGrid({ 
				url:'<%= request.getContextPath()%>/consultarSolicitudesPericialesCancelado.do?tipoSolicitud=<%=TiposSolicitudes.DICTAMEN.getValorId()%>&area=1&multiRol=1', 
				datatype: "xml", 
				colNames:['Número de Folio','Número de Caso','Número','Nombre del Solicitante','Especialidad de Perito','Fecha de Solicitud','Fecha Última Modificación' ], 
				colModel:[ 	{name:'NumeroFolio',index:'1', width:200},
							{name:'NumeroCaso',index:'2', width:200},
				...
				pager: jQuery('#pagerGridSolicitudesDeDictamenCanceladas'),
				rowNum:10,
				rowList:[10,20,30],
				autowidth: true,
				width:"100%",
				...
				ondblClickRow: function(rowid) {
					/*mandando variales/*ByYolo*/
 					var ret2 = jQuery("#gridSolicitudesDeDictamenCanceladas").jqGrid('getRowData',rowid);
 					var numidExpedienteDoc=ret2.NumeroExpediente;
//  					designarPeritoTest("true","true",numidExpedienteDoc,rowid,"NA",1);
 					visorCanceladasPericiales("true","true","true",numidExpedienteDoc,rowid,"NA",1);
 					/*fin/*ByYolo*/
// 					designarPerito(rowid,"NA",1);
				},				
				sortorder: "desc"
			}).navGrid('#pagerGridSolicitudesDeDictamenCanceladas',{edit:false,add:false,del:false});

			//cambia la variable a falso para ya no dibujar el grid, solo recargarlo
			firstGridSolicitudesDeDictamenCanceladas=false;
		}
		else{
			jQuery("#gridSolicitudesDeDictamenCanceladas").jqGrid('setGridParam', {url:'<%= request.getContextPath()%>/consultarSolicitudesPericialesCancelado.do?tipoSolicitud=<%=TiposSolicitudes.DICTAMEN.getValorId()%>&area=1&multiRol=1',datatype: "xml" });
			$("#gridSolicitudesDeDictamenCanceladas").trigger("reloadGrid");			
		}
		
		//muestra este grid y oculta los demas
		ocultaMuestraGrids('gridSolicitudesDeDictamenCanceladas');
	...
	function cargaGridSolicitudesDeDictamenTerminadas(){
		...
		if(firstGridSolicitudesDeDictamenTerminadas == true){
			...
			jQuery("#gridSolicitudesDeDictamenTerminadas").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['Número de Folio','Número de Caso','Número','Nombre del Solicitante','Especialidad de Perito','Fecha de Solicitud','Fecha de Cierre' ], 
				colModel:[ 	{name:'NumeroFolio',index:'1', width:200},
				           	{name:'NumeroCaso',index:'2', width:200},
				           	{name:'NumeroExpediente',index:'3', width:200},				           	
				           	{name:'NombreSolicitante',index:'4', width:230},
	...
	function cargaGridSolicitudesDeAsesoriaNoAtendidas(){
		...
		if(firstGridSolicitudesDeAsesoriaNoAtendidas == true){
			...
			jQuery("#gridSolicitudesDeAsesoriaNoAtendidas").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['Número de Folio','Número de Caso','Número','Nombre del Solicitante','Especialidad del Asesor','Fecha Límite' ], 
	...
	function cargaGridSolicitudesDeAsesoriaProceso(){
		...
		if(firstGridSolicitudesDeAsesoriaEnProceso == true){
			...
			jQuery("#gridSolicitudesDeAsesoriaEnProceso").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['Número de folio','Número de caso','Número','Nombre del solicitante','Especialidad del asesor','Fecha Límite','Fecha ultima de modificación' ], 
				colModel:[ 	{name:'NumeroFolio',index:'1', width:150},				           	
				           	{name:'NumeroCaso',index:'2', width:150},
				           	{name:'NumeroExpediente',index:'3', width:150},
				           	{name:'NombreSolicitante',index:'4', width:200},
	...
	function cargaGridSolicitudesDeAsesoriaTerminadas(){
		...
		if(firstGridSolicitudesDeAsesoriaTerminadas == true){
			...
			jQuery("#gridSolicitudesDeAsesoriaTerminadas").jqGrid({ 
				...
				datatype: "xml", 
				colNames:['Número de Folio','Número de Caso','Número','Nombre del Solicitante','Perito Designado','Especialidad','Fecha Límite','Fecha Cierre' ], 
				colModel:[ 	{name:'NumeroFolio',index:'1', width:150},
				           	{name:'NumeroCaso',index:'2', width:150},
				           	{name:'NumeroExpediente',index:'3', width:150},
				           	{name:'NombreSolicitante',index:'4', width:200},			           	
					...
					<ul id="seccion1tree" class="filetree">
						<li class="closed" ><span class="folder">Periciales</span>
							<ul>
							   ...
							   <li><span><img src="<%=request.getContextPath()%>/resources/css/check.png" width="16" height="16"  /><a id="pericialesCancelada" onclick="cargaGridSolicitudesDeDictamenCanceladas();">Canceladas</a></span></li>
							</ul>			
						</li>
					</ul>
				</div>
...
<!-- 					/*tabla para Canceladas/*ByYolo*/ -->
				<!--Espacio para el grid con las solicitudes periciales terminadas o cerradas-->
					<div id="divGridSolicitudesDeDictamenCanceladas">
						<table id="gridSolicitudesDeDictamenCanceladas"></table>
						<div id="pagerGridSolicitudesDeDictamenCanceladas"></div>
					</div>	
