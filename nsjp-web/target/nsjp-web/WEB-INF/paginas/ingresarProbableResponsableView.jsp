...
<% 
	...
	String muestraDetenido=null;
	
	Boolean esAtpenal = false;
	...
	if(usuario!=null &&
   	...
   	if (rolId.equals(Roles.ATPENAL.getValorId())){
...
<script type="text/javascript">
		...
		if( idElemento == 'undefined' ) {
			idElemento	= '<%=request.getSession().getAttribute("idImputadoDef")%>';
		}
	  ...
	  function bloqueaCamposTablaUAVDOpsHide(bandera)
	  {
		  if(parseInt(bandera)==0)
	  	{
			   	 ...
			   	 $("#btnPResponsableReincidente").attr('disabled','disabled');
			   	 $("#chkPResponsableDetenido").attr('disabled','disabled');
	  	...
	  	else
	  	{
		       	 ...
		       	 $("#btnPResponsableReincidente").attr('disabled','');
 ...
 function avilitaDatos(){
	   ...
	   */
		function muestraDatosCondicion(xml){
			...
			if($(xml).find('esReincidente').text() == "true"){
				 $("#btnPResponsableReincidente").attr('checked','checked');
			}
			else if($(xml).find('esReincidente').text() == "false"){
				$("#btnPResponsableReincidente").attr('checked',false);
			}
			if($(xml).find('desconocido').text() == "true"){	
				$("#btnPResponsableDesconocido").attr('checked','checked');
				deshabilitaDatosGeneralesDomicilio();
 ...
 */
		...
		*/
		...
		function alertDinamico2(textoAlert, id){
			...
			if(nombreGeneralOP!="" && desconocidoOp!="true" ){	
			  if(isnumeroDecimal(estatura) && isnumeroDecimal(peso)){
				...
				if(tipoResponsable == 1){//Tipo de responsable fisico
					...
					params+='&esReincidente=' + $('#btnPResponsableReincidente').is(':checked');///Aqui
					...
					if(estaDetenido && ($('#idFechaDateLapso').val() == '' || $('#idFechaDateLapso2').val() == '')) {
						...
						if(estaDetenido){
				    		...
				    		if(horaInicio < 6 || horaFin < 6 ){
				    			customAlert("Las horas son incorrectas. Favor de checarlas");
							}else{
						...
						}else{// si las fechas estan correctas asi como las horas ingresa al individuo
				...
				if(tipoResponsable == 1){//Tipo de responsable fisico
					...
					params+='&idCondicion=' + $(':radio[name=radio]:checked').val();
					params+='&esDesconocido=' + $('#btnPResponsableDesconocido').is(':checked');
					params+='&esReincidente=' + $('#btnPResponsableReincidente').is(':checked');
					params+='&esMenorDeEdad=' + $('#chkPResponsableMenor').is(':checked');
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="0">
					<tr>
						...
						<table width="12%" border="0" cellspacing="0" cellpadding="0"
							...
							<tr>
								<td align="left">Con Antecedente</td>
								<td><input type="checkbox"
									name="btnPResponsableReincidente "
									id="btnPResponsableReincidente" value="D" /></td>
							</tr>
			...
			<tr valign="top">
				...
				<table width="100%" border="0">
					<tr valign="top">
						...
						<input type="button" class="btn_mediano" id="btnBuscarReincidencia"	value="Buscar Antecedente"	onclick="cargaGridReincidenciaElemento()" /> 
...
<!-- Se cambia etiqueta de Reincidencia por Antecedente -->
<div id="dialog-Reincidencia" title="Antecedente(s) asociado(s) al elemento">
	<table id="gridReincidenciaElemento"></table>
	<div id="paginadorReincidenciaElemento"></div>
</div>

<div id="dialog-ReincidenciaCaso" title="Antecedente(s) registrado(s) al elemento">
