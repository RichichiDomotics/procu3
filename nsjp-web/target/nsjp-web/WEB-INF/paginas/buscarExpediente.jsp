...
function enableControls(tipo){
	...
	/*Funcion que evalua si trae involucradoid/*EnableIT ByYolo*/
	function detEviInvo(id, numExpediente, involucradoId){
	
	if (rolUsuario == "null"){
		rolUsuario = rolUserSesion;
	}
	
	if(rolUsuario == "defensor" || rolUsuario=="coordinadorDef"){
			idWindowNuevaDenuncia++;
			var ingresoDenuncia = true;
			$.newWindow({id:"iframewindowCarpInvNuevaDenuncia"+idWindowNuevaDenuncia, statusBar: true, posx:0,posy:0,width:1430,height:670,title:"Resumen: ", type:"iframe"});
			$.updateWindowContent("iframewindowCarpInvNuevaDenuncia"+idWindowNuevaDenuncia,'<iframe src="<%= request.getContextPath() %>/BusquedaExpediente.do?abreenPenal=abrPenal&ingresoDenuncia='+ingresoDenuncia+'&involucradoId='+involucradoId +'&idNumeroExpediente='+id+'" width="1430" height="670" />');
			$("#" +"iframewindowCarpInvNuevaDenuncia"+idWindowNuevaDenuncia+ " .window-maximizeButton").click();
	}else{
		detEvi(id,numExpediente)
	}
	}
	
	/*Funcion que evalua si trae involucradoid y caso/*EnableIT ByYolo*/
	function detEviInvo(idRow, numExpediente, idImputadoDef, numeroCaso){
	...
	if(rolUsuario=="defensor" || rolUsuario=="coordinadorDef"){
			...
			var ingresoDenuncia = true;
			$.newWindow({id:"iframewindowCarpInvNuevaDenuncia"+idWindowNuevaDenuncia, statusBar: true, posx:0,posy:0,width:1430,height:670,title:"Resumen: ", type:"iframe"});
			$.updateWindowContent("iframewindowCarpInvNuevaDenuncia"+idWindowNuevaDenuncia,'<iframe src="<%= request.getContextPath() %>/visorExpedienteDefensoria.do?idNumeroExpediente='+numExpediente+'&numeroCaso='+numeroCaso+'&idImputadoDef='+idImputadoDef+'" width="1140" height="400" />');
			$("#" +"iframewindowCarpInvNuevaDenuncia"+idWindowNuevaDenuncia+ " .window-maximizeButton").click();	
			

	}else{
		alert('brincocondicion');
		detEvi(idRow,numExpediente)
	}
	}
	...
	function detEvi(id, numExpediente) 
	{
		...
		if(rolUsuario == "adminat"){
			...
			$.newWindow({id:"iframewindowRegistraDatosPersona", statusBar: true, posx:200,posy:50,width:1140,height:450,title:"Constancia de Hechos Exp::  "+tituloVentana, type:"iframe"});
