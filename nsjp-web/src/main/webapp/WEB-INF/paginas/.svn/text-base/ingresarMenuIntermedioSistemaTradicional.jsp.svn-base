		...
		$(document).ready(function() {
			...
			*/ 						
			...
			jQuery("#gridDetalleFrmPrincipal").jqGrid({ 
				...
				datatype: "xml",
				colNames:['Folio','&Aacute;rea del responsable','Fecha de la actividad','Nombre de la actividad','Tipo de documento','Nombre de Documento','Fecha del documento', 'Documento', 'Es Parcial', 'Tipo Actividad Id'],
				...
				onSelectRow: function(id){
					...
					if(retd.EsParcial){
						...
						if(noEsParcial > 0){//"No es parcial"
							...
							if(retd.TipoActividadId == '<%=Actividades.CIERRE_DE_INVESTIGACION.getValorId()%>'){
								//Es el documento para el coordinador de consignacion
								//enviarCoordinadorGuardadoParcial(id);
							}
							else{//Es Definitivo y muestra el PDF
...
// 			se comenta la linea de abajo y se agrega el evento onclick en html /*Enable IT/*ByYolo*/
...
// 				/*se habilita solo este boton/*Enable IT ByYolo*/
				$("#btnCadCusConsultaCadCus").attr("disabled",false);
...
//	 	Mostrando cadena de custodia sin opciones a modificacion /*ByYolo*/
		function consultarRegCadenaCustodiaLectura()
		{
			idWindowAsntarRegCadCus++;
...
<%-- 		    $.updateWindowContent("iframewindowCnsltrRegCadCus" + idWindowAsntarRegCadCus,'<iframe src="<%= request.getContextPath() %>/consultarCadenaCustodiaDirector.do?consultaCadena=1&numeroExpediente='+numExpediente +'&operacion=2'+'&pantallaSolicitada='+pantallaSolicitada +'&idExpediente='+idExpedienteDoc+'" width="1100" height="760" />'); --%>
		    $.updateWindowContent("iframewindowCnsltrRegCadCus" + idWindowAsntarRegCadCus,'<iframe src="<%= request.getContextPath() %>/consultarCadenaCustodiaDirector.do?consultaCadena=1&numeroExpediente='+numeroExpediente +' " width="1000" height="600" />');
		}
			...
			else if(actividad=='<%= Actividades.GENERAR_CONVENIO_DE_CONCILIACION_MEDIACION.getValorId() %>')
			{
 			...
 			//USADAS PARA CONSIGNACION
				...
				if(estatusNumExp == '<%=EstatusExpediente.EN_TRAMITE.getValorId()%>'){
					...
					*/
					$.newWindow({id:"iframewindowGenerarDocumento", statusBar: true, posx:200,posy:50,width:1140,height:400,title:"Dirigir a Coordinador de Consignacion", type:"iframe"});//Se agrega la bandera para cambiar de estatus
				    $.updateWindowContent("iframewindowGenerarDocumento",'<iframe src="<%= request.getContextPath() %>/generarDocumentoSinCaso.do?formaId='+formaID+'&numeroUnicoExpediente='+numeroExpediente+'&cambiarEstatus='+true+'&ocultarGuardadoParcial='+true+'" width="1140" height="400" />');
				}				
				...
				$.newWindow({id:"iframewindowGenerarDocumento", statusBar: true, posx:200,posy:50,width:1140,height:400,title:"Dirigir a Coordinador de Consignacion", type:"iframe"});//Se agrega la bandera para cambiar de estatus
			    $.updateWindowContent("iframewindowGenerarDocumento",'<iframe src="<%= request.getContextPath() %>/generarDocumentoSinCaso.do?formaId='+formaID+'&numeroUnicoExpediente='+numeroExpediente+'&actividad='+actividad+'&ocultarGuardadoParcial='+true+'" width="1140" height="400" />');
			}			
			else{
				...
				if(actividad=='<%= Actividades.DIRIGIR_A_LA_UNIDAD_DE_SOLUCION_DE_CONTROVERSIAS.getValorId() %>'){			
					...
					if(existeDelitoGraveEnExpediente != ""){
						if( existeDelitoGraveEnExpediente == "false"){						
							...
							registrarActividadExpediente(actividad,estatusId,0);
			     			$.newWindow({id:"iframewindowGenerarDocumento", statusBar: true, posx:200,posy:50,width:1140,height:400,title:""+titulo, type:"iframe"});
			    		    $.updateWindowContent("iframewindowGenerarDocumento",'<iframe src="<%= request.getContextPath() %>/generarDocumentoSinCaso.do?formaId='+formaID+'&numeroUnicoExpediente='+numeroExpediente+'&ocultarGuardadoParcial='+true+'" width="1140" height="400" />');
						}
								}
							}
						else{
					...
					if(actividad =='<%= Actividades.DIRIGIR_A_LA_UNIDAD_DE_FISCALES_INVESTIGADORES.getValorId() %>'){
						...
						registrarActividadExpediente(actividad,estatusId,0);
		     			$.newWindow({id:"iframewindowGenerarDocumento", statusBar: true, posx:200,posy:50,width:1140,height:400,title:""+titulo, type:"iframe"});
		     			/*Se habilita el boton de guardado parcial a petición de OAXACA/*ByYolo*/
		    		    $.updateWindowContent("iframewindowGenerarDocumento",'<iframe src="<%= request.getContextPath() %>/generarDocumentoSinCaso.do?formaId='+formaID+'&numeroUnicoExpediente='+numeroExpediente+'&ocultarGuardadoParcial='+false+'" width="1140" height="400" />');						
					}
				}
             }
		}
...
//***************************************************COMIENZAN FUNCIONES PARA ACTUACIONES CONSIGNACION***********************************************/
	...
	*/
	function enviarCoordinador(formaID){
		...
		*/
		...
		if(cveFuncionario != ""){				
			$.newWindow({id:"iframewindowGenerarDocumento", statusBar: true, posx:200,posy:50,width:1140,height:400,title:"Dirigir a Coordinador de Consignacion", type:"iframe"});
		    $.updateWindowContent("iframewindowGenerarDocumento",'<iframe src="<%= request.getContextPath() %>/generarDocumentoSinCaso.do?formaId='+formaID+'&numeroUnicoExpediente='+numeroExpediente+'&enviarCoordConsignacion='+true+'&cveFuncionario='+cveFuncionario+'&ocultarGuardadoParcial='+true+'" width="1140" height="400" />');
		}
		else{
	...
	*/
	function recargarBandejaIndexAgenteMpSt(){
		...
		window.parent.cargaGridExpedientesPorEstatus(estatusRecargaExpediente,"fechaini","fechafin");
	}
...
//					alertDinamico("El delito principal debe ser un delito grave,\n por favor revise su selección");
	...
	<!-- /*FIN DE LOS script que contiene primera funcion que tiende a fallar, no afecta funcionalidad*ByYolo*/ -->
...
<!--TERMINAN TABS INFERIORES DE ACTUACIONES-->
		...
		<div id="tabs-10" class="tabTabsCadCus">
				 ...
				 <input type="button" class="btn_Generico" id="btnCadCusConsultaCadCus"  onclick="consultarRegCadenaCustodiaLectura()" style="width: 250px;" value="Consultar custodia de evidencias"/><br/><br/>    
