...
<script type="text/javascript"
	src="<%=request.getContextPath()%>/js/bloqueaTecla.js"></script>
...
var resRad;
	...
	var valorSI = "<div style=\"background-color: #f2f2f2; color:#393939;\">Si </div>";
	 ...
	 $(document).ready(function() {
		...
		//Enable JC
		var tipoSolCompartida = <%=request.getParameter("tipoSolCompartida")%>;
		...
		jQuery("#gridDetalleFrmPrincipal").jqGrid({ 
					...
					colModel:[ 	{name:'folio',index:'folio', width:80},
								...
								{name:'Nombre',index:'nombre', width:255},
					           	...
					           	{name:'Documento',index:'documento', width:170,  hidden:true}
								],
					...
					onSelectRow: function(id){
						...
						if(retd.EsParcial){
							var noEsParcial = retd.EsParcial.indexOf('Definitivo');
							if(noEsParcial > 0){//No es parcial
				    		    ...
				    		    $.updateWindowContent("iframewindowGenerarDocumento",'<iframe src="<%= request.getContextPath() %>/generarDocumentoSinCaso.do?documentoId='+id+'&numeroUnicoExpediente='+numeroExpedienteId+'" width="1140" height="400" />');
							}
						}
					},
					sortorder: "desc"
		...
		//Enable IT JC
		if(tipoSolCompartida > 0){
			idTipoSolicitud = tipoSolCompartida;
		}
		if(pantalla==5){
...
function popopAsistencia(rowid){
	...
	function documentos(){
		 jQuery("#gridDetalleFrmPrincipal").jqGrid('setGridParam',
					...
					datatype: "xml" ,
					success: function(xml){
						 //alert('<%=request.getSession().getAttribute(
	...
	*/
		 ...
		 $.ajax({
			...
			success: function(xml){
				if($(xml).find('respuesta').find('bandera').text() == '1'){
					...
					alert("No se logró asignar el expediente");
				}
			}
		 });
		}
	...
	function registrarActividadExpediente(actuacionID,estatusId,banderaMensaje, cveFuncionarioAsignado)
	{
			...
			$.ajax({
				...
				success: function(xml)
				{
					if($(xml).find('flag').text() == "true"){
						alert("El Expediente ya se encuentra asignado a esta persona.");
					}
					else if(parseInt($(xml).find('actividadDTO').find('actividadId').text()) > 0)
					{
 ...
 --%>
		...
		function seleccionaActuacionTrabajoSocial()
		{
			...
			if(usaeditor== "true")
			{
				...
				else if(actividad=='<%=Actividades.SEGUIMIENTO_A_ESTUDIO_SOCIOECONOMICO
					.getValorId()%>')
				{
					registrarActividadExpedienteUsr(actividad,estatusId,0);
...
/*****************************************************************************************************/		
	function ejecutaActuacion(selected, confActividadId, actividad, formaID, titulo, usaeditor, estatusId){
	  	if(usaeditor== "true"){	
	  		registrarActividadExpedienteEna(actividad,estatusId,0);
    ...
    function registrarActividadExpedienteEna(actuacionID,estatusId,banderaMensaje){
    	//Cambia la actividad del expediente
    	$.ajax({
			type: 'POST',
...
<body>
	<div id="tabsPrincipal">
		<ul>
			<li><a href="#tabsconsultaprincipal-1">Resumen</a>
			</li>
			<!-- Enable IT: Se cambia etiqueta Involucrado por Usuario -->
			<li><a href="#tabsconsultaprincipal-2">Usuario</a>
			</li>

			<li class="tabPsicologica"><a href="#tabsconsultaprincipal-4">Atención
					Psicológica</a>
			</li>
			<li class="tabSocial"><a href="#tabsconsultaprincipal-6">Trabajo
					Social</a>
			</li>
			<li class="tabJuridica"><a href="#tabsconsultaprincipal-7">Atención
					Jurídica</a>
			</li>
			<li id="tabNotas"><a href="#tabsconsultaprincipal-5">Notas</a>
			</li>
			<li class="tabTabsDocs"><a href="#tabsconsultaprincipal-8"
				onclick="documentos()">Documentos</a>
			</li>
		...
		<div id="tabsconsultaprincipal-1">
			<table width="1042px" border="0">
				<tr>
					<td width="50%" align="right"><span id="tdFuncionario">Seleccione
							el funcionario que atenderá la solicitud</span>
					<td>
					<td width="30%" align="right"><select
						id="cbxFuncionariosAyuda" style="width: 250px">
							<option value="0">-Seleccione-</option>
					</select>
					<td>
					<td width="20%" align="right"><input type="button"
						class="btn_mediano" value="Asignar Ayuda"
						onclick="asignarAyuda();" id="btnSolicitarAyuda" />
					<td>
				</tr>
			</table>
			<div style="width: 1042px; height: 490px;" class="back_hechos">
				...
				<table width="50%">
					<tr>
						...
						<td id="spanTipoSolicitud"></td>
					</tr>
					<tr>
						<td width="10%"></td>
						<td><b>Nombre de usuario : </b></td>
						<td id="spanNombreVictima"></td>
					</tr>
					<tr>
						<td width="10%"></td>
						<td><b>Delito : </b></td>
					...
					<tr>
						<td width="10%"></td>
						<td><b>AMP que solicit&oacute; la ayuda : </b></td>
						<td id="spanAMPSolicito"></td>
					</tr>
					<tr>
						<td width="10%"></td>
						<td><b>Área que solicit&oacute; la ayuda : </b></td>
						<td id="spanAreaSolicito"></td>
					</tr>
					<tr>
						<td width="10%"></td>
						<td><b>Fecha de creaci&oacute;n del Expediente : </b></td>
						<td id="spanFechaCreacion"></td>
					</tr>
					<tr>
						<td width="10%"></td>
						<td><b>Estatus de Expediente : </b></td>
						<td id="spanEstatusExpediente"></td>
					</tr>
					<tr>
						<td width="10%"></td>
						<td><b>Estatus de Actuaciones : </b></td>
						<td id="spanEstatusActuaciones"></td>
					</tr>
					<tr>
						<td width="10%"></td>
						<td><b>Responsable del Trámite : </b></td>
						<td id="spanResponsableTramite"></td>
					</tr>
				</table>
			</div>
		</div>
		...
		<div id="tabsconsultaprincipal-2" class="tabs-bottom">
			<ul>
				<li><a href="#tabschild-2">Víctima</a>
				</li>
				<li><a href="#tabschild-3"><bean:message
							key="probableResponsableTitulo" />
				</a>
				</li>
			</ul>
			<div id="tabschild-2">
			...
			<div id="tabschild-3">
				<div style="width: 1042px; height: 490px;" class="back_hechos">
					<table width="25%" cellpadding="0" cellspacing="0"
						id="tblProbableResponsable" style="padding: .5cm;">
						<tr>
							<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a
								id="nuevoProbResponsable"></a>
							</td>
						</tr>
					</table>
				</div>
		...
		<div id="tabsconsultaprincipal-4">
			<table width="100%" border="0">
				<tr>
					...
					<td width="50%"><select id="cbxAccionesTab"
						style="width: 470px">
							<option value="-1">-Seleccione-</option>
					</select></td>
					<td width="30%" align="left"><input type="button"
						value="Agregar Nota de Evolución" class="btn_Generico"
						id="btnAgregarNotaEvaluacion" />
					</td>
				</tr>
				<tr>
					<td width="10%">&nbsp;</td>
					...
					<td width="30%" align="left">
						<button value="Adjuntar documento" id="btnAdjuntarDocumentoPsico"
							class="btn_Generico"
							onclick="abreVentanaAdjuntarDocumentoAExpediente()">Adjuntar
							documento</button></td>
				</tr>
				<tr>
					<td width="10%">&nbsp;</td>
					<td width="60%" colspan="2">
						...
						<div id="pagerGridAtencionPsicologica"></div></td>
					<td width="30%">&nbsp;</td>
				</tr>
			</table>
		...
		<div id="tabsconsultaprincipal-5">
			<table width="25%" cellpadding="0" cellspacing="0"
				id="tablaNotasExpediente">
				<tr>
					<td>&nbsp;&nbsp;&nbsp;<input type="button"
						class="btn_Generico" value="Generar Nota" id="botonGuardarNotas"
						onclick="notaExpediente(0);" />
					</td>
				</tr>
			</table>
		</div>
		<div id="tabsconsultaprincipal-6">
			<table width="100%">
				<tr>
					<td width="10%">&nbsp;</td>
					<td width="10%">Actividades:</td>
					<td width="50%"><select id="cbxAccionesTabTS"
						style="width: 470px"> 
						
							<option value="-1">-Seleccione-</option>
					</select></td>
					<td align="left">
						<button value="Adjuntar documento" id="btnAdjuntarDocumentoSocial"
							class="btn_Generico"
							onclick="abreVentanaAdjuntarDocumentoAExpediente()">Adjuntar
							documento</button></td>
				</tr>
		...
		<div id="tabsconsultaprincipal-7">
			<table width="100%" border="0">
				<tr>
					<td width="10%">&nbsp;</td>
					<td width="10%">Actividades:</td>
					<td width="50%"><select id="cbxAccionesTabJ"
						style="width: 470px">
							<option value="-1">-Seleccione-</option>
					</select></td>
					<td align="left">
						<button value="Adjuntar documento" id="btnAdjuntarDocumentoJur"
							class="btn_Generico"
							onclick="abreVentanaAdjuntarDocumentoAExpediente()">Adjuntar
							documento</button></td>
				</tr>
		...
		<div id="tabsconsultaprincipal-8" class="tabTabsDocs">
			...
			<form name="frmDoc2"
				...
				method="post">
				<input type="hidden" name="formaId" /> <input type="hidden"
					name="numeroUnicoExpediente" />
			</form>
			<table id="tableDocs">
				<tr>
	...
	<div id="dialogAsistencia" style="display: none;">
		<table width="100%" border="0">
			<tr>
				<td width="3%">Si</td>
				<td width="48%"><input type="radio" name="radioAsistio"
					id="radioAsistioSi" value="1" /></td>
				<td width="4%">No</td>
				<td width="45%"><input type="radio" name="radioAsistio"
					id="radioAsistioNo" value="2" />
				</td>
			</tr>
		</table>
	</div>
