...
<%-- 	var numeroCaso='<%=request.getParameter("numeroCaso")!=null?request.getParameter("numeroCaso"):"Sin numero"%>'; --%>
		...
		*/
		function crearPdf(){
			if(validadDatosSolicitud() == 1){
				...
				/*se envia el parametro de para hacer vincular este expediente con el cNumeroGeneralCaso/*ByYolo*/
				document.frmDoc.action ="<%= request.getContextPath() %>/GenerarDocumentoNotificacionAyudaPsicologicaUAVD.do?&numeroCaso='<%=request.getParameter("numeroCaso")!=null?request.getParameter("numeroCaso"):"Sin numero"%>';";
				//separamos los ids
		...
		function validadDatosSolicitud(){
			totalDestinatario = jQuery('#gridUsuarios').jqGrid('getGridParam','records');
			if(totalDestinatario >0 && idFunDestinatario!="" && idFunDestinatario.length>0 ){
				enviaDatosSolicitud()
				return 1;
			}else{
