...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		/*se mueve la llamada del metodo devido a qaue ambos son asincronos y el metodo resumen depende de los datos de 
		consultarInformePolicialHomologadoPorFolio/*Enable IT ByYolo*/
// 		resumen();
		...
		$.ajax({
	  		type: 'POST',
	  	  	...
	  	  	success: function(xml){
	  			...
	  			/*se reubica metodo/*Enable IT ByYolo*/
	  			resumen();
	  	  	}
		});
	}
	...
	**/
	...
	function recuperaDatosGenerales()
	{
	    ...
	    /***********************************/
	    /*			agregando oficial y sector a los parametros*/
	    lsDatosGenerales+="&oficial="+$("#datosGeneralesCmpOficial").val();
	    lsDatosGenerales+="&sector="+$("#datosGeneralesCmpSector").val();
	    /***********************************/
		...
		if($("#datosGeneralesCmpNumeroTransporteOf").val() == "" || $("#datosGeneralesCmpAsunto").val() == "" ||
		   ...
		   ($("#chkOperativo").is(':checked') && ($("#datosGeneralesCmpNombreOperativo").val() == "" || 
			...
			$.ajax({								
	  		  ...
	  		  /*se returna ok/*ByYolo*/
	  		  return "ok";
		}
	}

	/*****
	Enable IT se habilita metodo para poder modificar el hecho y generar el pdf de manera correcta 
	****/
...
// 						$('#motivoCmpSubtipoEvento').append('<option value="' + $(this).find('claveDelito').text() + '">' + $(this).find('nombre').text() + '</option>');
	...
	function consultarHecho(idHecho, numeroExpedienteId) {	
		idWindowIngresarHechos++;
		$.newWindow({id:"iframewindowHecho" + idWindowIngresarHechos, statusBar: true, posx:200,posy:50,width:1050,height:600,title:"Hechos", type:"iframe"});
	    <%-- $.updateWindowContent("iframewindowHecho" + idWindowIngresarHechos,'<iframe src="<%= request.getContextPath() %>/IngresarHechos.do?iphFuncionalidadHidden=true&numeroExpedienteId='+numeroExpedienteId +'&idCalidad=DENUNCIANTE&idLugar=1&idHecho='+idHecho +' " width="1050" height="600" />'); --%>
	...
	*/
	...
	function adjuntardoc(valor) {
		...
		if (numeroExpedienteId != "undefined" && numeroExpedienteId != "null"
				&& numeroExpedienteId != "") {
			$.newWindow({
				id : "iframewindowAdjuntarDocumento",
				statusBar : true,
				posx : 70,
				posy : 50,
				width : 450,
...
<body>
	...
	<table width="100%" class="back_pleca_h">
		<tr>			
			<td align="right">
					 <input type="button" value="Guardar" id="btnIPHGuardadoParcial" class="btn_Generico" onclick="guardarDatosGeneralesIPH()"/>
					 <!--
					 	Enable It se cambia el class del boton para poder generar informe correcto despues de modificar el hecho 
					  -->
	...
	<div id="tabsconsultaprincipal">
		<ul>
			...
			<!-- Enable IT : ocultando pestaña de detencion
							...
							<td valign="top">
								<table>
									...
									<tr>
										...
										<td><input type="text" style="width: 180px;" maxlength="50" id="datosGeneralesCmpOficial"/></td>
										Enable IT agregando boton para adjuntar documentos
										<td><input type="button" align="left" value="Adjuntar Documento" id="adjuntardoc" onclick="adjuntardoc('1');" class="btn_Generico"/>
										<td>&nbsp;</td>
									</tr>
									<tr>
									...
									<!-- <tr>
										...
										<td><input type="text" style="width: 180px;" maxlength="50" id="datosGeneralesCmpDirigidoA" /></td>
										<td>&nbsp;</td>
									</tr> -->
									<tr>
		...
		<div id="tabsconsultaprincipal-9" class="tabTabsCadCus">
				 ...
				 <input type="button" class="btn_grande" id="btnCadCusConsultaCadCus" style="width: 250px;" value="Consultar cadena de custodia"/><br/><br/>    
  				 <!-- eNABLE IT: Se ocultan botones ya que no tienen funcionalildad -->
