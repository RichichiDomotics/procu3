   ...
   function guardado(){
		...
		if(vMotivoDEATT != 0){
	   	...
	   	if((vSolicitanteDEATT==0)&&(vParaQuienDEATT==0)&&(vMotivoDEATT==0)){
	   		...
	   		$.ajax({
				type: 'POST',
				url: '<%= request.getContextPath()%>/obtenerConfActividadDocumento.do?idConf='+7,
				data: '',
				dataType: 'xml',
				async: false,
				success: function(xml){
	...
	function actualizaEstatusSolicitudDefensoria(textoRecuperado){
		$.ajax({
			type: 'POST',
    ...
    function generarDocumentoSinCaso() {
		...
		document.frmDoc2.textoRecuperado.value			= textoRecuperado;
		document.frmDoc2.numeroUnicoExpediente.value = numExpediente1;
...
//    }
	...
	*/
	...
	<form name="frmDoc2" action="<%= request.getContextPath() %>/GenerarDocumentoDirecto.do" method="post">
		...
		<input type="hidden" name="textoRecuperado" />
