...
public class ParametrosDocumentoDTO extends GenericDTO {
	...
	private String cnumeroGeneralCaso;
	...
	public InvolucradoDTO getPrimerDenunciante(){
		if(denunciantes != null && !denunciantes.isEmpty()){
			//Enable IT JC. Muestra el denunciante mas reciente
			int numDenunciantes = denunciantes.size();
			return denunciantes.get(numDenunciantes-1);
			//return denunciantes.get(0);
		}else{
			if(denunciantesOrganizacion != null && !denunciantesOrganizacion.isEmpty()){
				int numDenOrg = denunciantesOrganizacion.size();
				return denunciantesOrganizacion.get(numDenOrg-1);
...
//    		Enable IT: Se corrige para obtener jueces
    		if(funcionario.getPuesto().getIdCampo().equals(Puestos.JUEZ.getValorId())){
    			sbFuncionarios.append(funcionario.getNombreCompleto()+"<br> ");
    		}
    	}
	...
	public String getDefensoresAudiencia(){
    	...
    	List<FuncionarioDTO> funcionarios = audiencia.getFuncionarios();
    	for(FuncionarioDTO funcionario : funcionarios){
//    		Enable IT: Se corrige para obtener defensores
	...
	/*
	 * Enable IT: se agrega método para obtener los MPs de la audiencia
	 * */
	public String getMpAudiencia(){
    	StringBuffer sbFuncionarios = new StringBuffer();
    	List<FuncionarioDTO> funcionarios = audiencia.getFuncionarios();
    	for(FuncionarioDTO funcionario : funcionarios){
    		if(funcionario.getTipoEspecialidad().getIdCampo().equals(TipoEspecialidad.MINISTERIO_PUBLICO.getValorId())){
    			sbFuncionarios.append(funcionario.getNombreCompleto()+"<br> ");
    		}
    	}
		return sbFuncionarios.toString();		
	...
	public String getCnumeroGeneralCaso() {
		return cnumeroGeneralCaso;
	}

	public void setCnumeroGeneralCaso(String cnumeroGeneralCaso) {
		this.cnumeroGeneralCaso = cnumeroGeneralCaso;
	}
