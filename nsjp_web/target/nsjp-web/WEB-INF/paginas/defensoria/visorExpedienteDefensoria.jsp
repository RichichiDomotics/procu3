...
<%
	...
	long idImputadoDef = (Long)(request.getSession().getAttribute("idImputadoDef"));
...
%>
	...
	*/
	...
	var iframewindowAbrirteoria = 1; // Se agrega variable para iframe de abrir teoria de caso
	...
	$(document).ready(function() {
		...
		$("#rdbMenuInterActEnvSol").bind("click",ocultaMuestraTblsActuaciones);
		$("#rdbMenuInterActCamEst").bind("click",ocultaMuestraTblsActuaciones);
	...
	function ocultaMuestraTblsActuaciones(){
	...
	function funcionalidadNoDisponible(){
		alertDinamico("Funcionalidad no Disponible");
	}
	...
	*/
	...
	function seleccionaActuacion(){
		...
		if (parametroConfirm == '1' &&
			...
			titulo, usaeditor, estatusId, habilitarTurno);
		}else{
			ejecutaActuacion(selected, confActividadId, actividad, formaID, titulo, usaeditor, estatusId, habilitarTurno);
		}
	}
	...
	function seleccionaActuacionEstatus(){
		...
		customConfirm('¿Está seguro? '+ selected2 + ' puede generar alguna solicitud o cambio de estatus.', 'Cambio Estatus/Solicitud',  
						function (){ ejecutaActuacion(selected, confActividadId, actividad, formaID, titulo, usaeditor, estatusId, habilitarTurno)});
	...
	function ejecutaActuacion(selected, confActividadId, actividad, formaID, titulo, usaeditor, estatusId, habilitarTurno){
		if(usaeditor== "false"){
			...
			} else if (actividad == '<%=Actividades.SOLICITAR_CARPETA_DE_INVESTIGACION.getValorId()%>'){
	...
	function solicitarCarpetaInvestigacion(){
		...
		if(cofirmacion){
			var parametros = "numeroExpedienteId="+idNumeroExpedienteOp;
			$.ajax({
	...
	*/
	...
	/*Se modifica la informacion a presentar sobre las audiencias*/
	 function gridAudiencia() {
			jQuery("#gridAudiencia").jqGrid({
				url:'<%= request.getContextPath()%>/consultarAudienciasDefensor.do?numExp='+'<%=numeroExpedienteIdOK%>',
						datatype: "xml",
						colNames:['Número de Expediente', 'No. Caso','Número de Causa','Tipo de Audiencia','Fecha de Audiencia','Sala','Juzgado' , 'Estatus'],  
						colModel:[{name:'numeroExpediente',	 index:'numeroExpediente', 		width:160},
						          {name:'caso',	 index:'caso', 		width:160, hidden:true},
						          {name:'numeroCausa',	 index:'numeroCausa', 		width:100},
					          	  {name:'tipo',index:'tipo', 	width:230},
					          	  {name:'fechaHora',	 index:'fechaHora', 	    width:120},
					          	  {name:'sala',index:'sala',width:50},
					          	  {name:'juzgado' ,index:'juzgado', 		width:100} ,
	...
	function pintaDatosResumen(detalle){
		...
		/*se comentan las lineas debido a que con etiquetas de Struts le insertan el dato/*Enable IT ByYolo*/
		//Numero de caso
...
// 		}
		...
		/*lineas sustituyen a las abajo comentadas.Se muestran el delito en la pestaña/*Enable IT ByYolo*/
		if (existe($(detalle).find('avisoDesignacionDTO').first().find('solicitudDefensor').find('delitos').first().find('delitoDTO').find('catDelitoDTO').first().find('nombre').text())){
			$("#delito").val($(detalle).find('avisoDesignacionDTO').first().find('solicitudDefensor').find('delitos').first().find('delitoDTO').find('catDelitoDTO').first().find('nombre').text());
		}else{
			$("#delito").val("---");
		}
...
<div id="tabedMenuExpedienteDefensoria">
	<ul>
		...
		<!-- Enable IT se oculta pestaña de diligencias , se agregan como actuaciones -->
		...
		<li id="tab_Actuaciones"><a href="#tabsconsultaprincipal-10" onclick="recargarActuaciones()" >Actuaciones</a></li>
