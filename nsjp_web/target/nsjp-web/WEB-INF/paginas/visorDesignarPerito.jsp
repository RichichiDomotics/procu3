...
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
	...
	var numidExpedienteDoc = '<%=request.getParameter("numidExpedienteDoc")%>';
	var numExpediente=parent.numExpediente;
	var deshabilitarPestanas = '<%=request.getParameter("deshabilitarPestanas")%>';
		...
		$(document).ready(function() {
			...
			numidExpedienteDoc = '<%=request.getParameter("numidExpedienteDoc")%>';
			/*En BD son llaves primarias y su relacion es 1 a 1 es por ello que son la misma/*ByYolo*/
			solicitudId = solicitudPericialId;
			/*Variable para ocultar Ventana*/
			deshabilitarPestanas = '<%=request.getParameter("deshabilitarPestanas")%>';
			deshabilitarBotonSP ='<%=request.getParameter("deshabilitarBotonSP")%>'
			habilitarDetalleCancelacion = '<%=request.getParameter("habilitarDetalleCancelacion")%>'
			
			if(deshabilitarPestanas=='true'){
				$("#tabsconsultaprincipal-2").hide();
				$("#liDesignacionPerito").hide();
			}
			
			$("#divObservaciones").hide();
			$("#divDetatalleCancelacion").hide();
			
			if(deshabilitarBotonSP=='true'){
// 				$("#buttonCancelarSolicitudPericial").hide();
// 				$("#divObservaciones").hide();
				$("#botonMostrarObservaciones").hide();
			}
			
			if (habilitarDetalleCancelacion=='true'){
				$("#divDetatalleCancelacion").show();
			}
	...
	function cargaGridDocumentosDigitalesPropios(){
		...
		if(primeraVezGridDocumentosDigitalesPropios == true){
			jQuery("#gridDocumentosDigitalesPropios").jqGrid({
				url:'<%=request.getContextPath()%>/consultarDocumentos.do?numidExpedienteDoc='+numidExpedienteDoc+'',
				datatype: "xml", 
			...
			jQuery("#gridDocumentosDigitalesPropios").jqGrid('setGridParam', {url:'<%=request.getContextPath()%>/consultarDocumentos.do?numidExpedienteDoc='+numidExpedienteDoc+'',datatype: "xml" });
	...
	*/
	function abrirDocsDigAsociadosASol(documentoAsocId){
		if(documentoAsocId != null && documentoAsocId != ""){
			...
			// $("#visorDocsFrame").attr("src","<%= request.getContextPath()%>/consultarArchivoDigitalIframe.do?archivoDigitalId="+documentoAsocId+"&inFrame=1");
			//Versión de Prueba
			$("#visorDocsFrame2").attr("src","<%= request.getContextPath()%>/consultarArchivoDigitalIframe.do?archivoDigitalId="+documentoAsocId+"&inFrame=true");
		}
		else{
	...
	function cancelarSolicitudesPericiales(){
		var observaciones = $('#observacionesCancelacionSP').val();
		var esActualizado =''; 
		
		$.ajax({
			type: 'POST',
			url: '<%= request.getContextPath()%>/cancelarSolicitudPericial.do?solicitudId='+solicitudId+'&observaciones='+observaciones+'',
			data: '',
			dataType: 'xml',
			async: false,
			success: function(xml){			
				esActualizado = $(xml).find('esActualizado').first().text()
...
//     yolotzinvar
    if($(xml).find('observaciones') != null){
    	$('#detalleCancelacion').val($(xml).find('observaciones').first().text());
 	}
...
<body>
	...
	<div id="tabsprincipalconsulta">
		<ul>
			...
			<li><a href="#tabsconsultaprincipal-5"  onclick="cargaGridDocumentosDigitales();">Solicitudes Periciales</a>
			</li>
			<li><a href="#tabsconsultaprincipal-4" onclick="cargaGridDocumentosDigitalesPropios()">Documentos</a>
			</li>
		...
		<div id="tabsconsultaprincipal-1" align="left">
			...
			<table width="950" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td align="right">
					<input type="button"  id="botonMostrarObservaciones" value="Cancelar Solicitud" class="btn_Generico" onclick="mostrarObservaciones()"/> 
					</td>				
				</tr>
			</table>
...
<!-- 			/*Agregando campo Observaciones/*ByYolo*/ -->

			<div id="divObservaciones">
			<fieldset style="width: 700px;">
			<legend>Cancelación de Solicitud: </legend>
			<table width="700" border="0">
				<tr>
					<td align="left">
				...
				<tr>
					<td align="justify">
						<textarea rows="5" cols="80" id="observacionesCancelacionSP"></textarea>
					</td>
<!-- 				</tr> -->
<!-- 				<tr> -->
					<td align="right">
						<input type="button" id="buttonCancelarSolicitudPericial" value="Enviar Cancelación" class="btn_Generico" onclick="cancelarSolicitudesPericiales();">
					</td>
				</tr>
			</table>
			</fieldset>				
...
<!-- 			/*Detalle de las observaciones/*ByYolo*/	 -->
			<div id="divDetatalleCancelacion">
				<fieldset style="width: 700px;">
					<legend>Detalle de la cancelacion: </legend>								
					<table>
						<tr>
							<td align="justify">
								<textarea rows="5" cols="80" disabled="disabled" id="detalleCancelacion" ></textarea>
							</td>
						</tr>
					</table>
				</fieldset>	
...
<!-- 			/*Fin campo observaciones/*ByYolo*/ -->
			
<!-- 		/*boton para cancelar Solicitud Pericial/*ByYolo*/ -->
<!-- 		    <div id="divCancelarSolicitudPericial"> -->
<!-- 		    <br> -->
<!-- 		    <br> -->
<!-- 		          <input type="button" id="buttonCancelarSolicitudPericial" value="Cancelar Solicitud Pericial" class="btn_Generico" onclick="cancelarSolicitudesPericiales();"> -->
<!-- 		    </div> -->
<!-- 		    /*Fin Boton/*ByYolo*/ -->
		...
		<div id="tabsconsultaprincipal-2" align="left">
			...
			<table width="1000" border="0" cellspacing="0" cellpadding="0">
		        ...
		        <tr>
		            ...
		            <td align="left" valign="middle">
		            	<div id="divNombrePeritoTxtField"><input type="text" id="txtFieldNombrePerito" style="width: 180px;" onkeypress="return soloLetrasNPunto(event,this.id);"/></div>
		        ...
		        <tr>
		            ...
		            <td align="left" valign="middle">
		            	<div id="divTxtFieldApPatPerito">
		            		<input type="text" id="txtFieldApPatPerito" style="width: 180px;" onkeypress="return soloLetrasNPunto(event,this.id);"/>
		            	</div>
		            </td>
		        </tr>
		        <tr>
		            ...
		            <td align="left" valign="middle">
		            	<div id="divTxtFieldApMatPerito">
		            		<input type="text" id="txtFieldApMatPerito" style="width: 180px" onkeypress="return soloLetrasNPunto(event,this.id);"/>
		            	</div>
		            </td>
		        </tr>
		        <tr>
...
<!-- 		comienza tab Solicitudes Periciales -->
					...
					<table width="1150"  height="530" border="0" cellspacing="0" cellpadding="0">
		              <tr>
		                ...
		                <td width="900" align="center" valign="top">
		               	  <iframe id='visorDocsFrame2' width="900" height="500" src="">		               	  
		               	  </iframe>
		                </td>
		              </tr>
		            </table>
...
<!-- 		termina tab Solicitudes Periciales -->

<!-- 		comienza tab Solicitudes Documentos -->
