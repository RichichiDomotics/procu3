...
import mx.gob.segob.nsjp.dto.traslados.TrasladoDTO;
...
public class AdministrarNumeroExpedienteServiceImpl implements AdministrarNumeroExpedienteService {
    ...
    public boolean asociarNumCarpetaASolicitud( ExpedienteDTO expedienteDTO, String numExpedientePadre_id, Long idSolicitud)throws NSJPNegocioException{
		...
		/*se obtiene el NumeroExpediente del Padre y se inserta al expediente hijo en UAVD folios like '%VictiDel%'/*byYolo*/
//		NumeroExpediente numeroExpedientePadre = numeroExpedienteDAO.obtenerNumeroExpediente(numExpedientePadre_id,null);
//		numeroExpediente.setNumeroExpedientePadre(numeroExpedientePadre);
		/*fin/*byYolo*/
	...
	public void asociarExpedienteAFuncionario(Long idNumeroExpediente, Long idFuncionario) throws NSJPNegocioException{
		...
		/*se le asigna la catuiespecializada para que los puedan visualizar el coordinadorAMP/*Enable IT ByYolo*/
		Funcionario funcionario= funcionarioDAO.read(idFuncionario);
		
		if(funcionario!=null)
			if(funcionario.getUnidadIEspecializada()!=null)
				if(funcionario.getArea().getJerarquiaOrganizacionalId().equals(Areas.UNIDAD_INVESTIGACION.parseLong()) ||
						funcionario.getArea().getJerarquiaOrganizacionalId().equals(Areas.COORDINACION_UNIDAD_INVESTIGACION.parseLong())){
					if(loNumExp!=null)
						if(loNumExp.getExpediente()!=null)
							if(loNumExp.getExpediente().getCatUIEspecializada()==null)
								loNumExp.getExpediente().setCatUIEspecializada(funcionario.getUnidadIEspecializada());
			}
				
		
		logger.debug("Clave Funcionario ::::::: " + loNumExp.getFuncionario().getClaveFuncionario());
		logger.debug("Expediente Id ::::::: " + loNumExp.getExpediente().getExpedienteId());
		if (idFuncionario != loNumExp.getFuncionario().getClaveFuncionario()){
			loNumExp.setFuncionario(new Funcionario(idFuncionario));
			numeroExpedienteDAO.update(loNumExp);
		}
		Expediente exp = expedienteDAO.read(loNumExp.getExpediente().getExpedienteId());
		...
		if (exp.getCaso()== null){
	        ...
	        logger.info("Nuevo CASO GENERADOOO ::::  " + casoDTO.getCasoId());
			exp.setCaso(new Caso(casoDTO.getCasoId()));
	...
	public ExpedienteDTO generarNuevoExpedienteUAVD(DelitoPersonaDTO delitoPersonaDTO) throws NSJPNegocioException{
        ...
        guardarInvolucradoEnExpediente(expBD,delitoPersonaDTO.getVictima(),Calidades.VICTIMA_PERSONA);
       	...
       	guardarDelitoService.guardarDelito(loLista, new ExpedienteDTO(expBD.getExpedienteId()), "0");
        return expBD;
	}
	...
	private void eliminarIdentificadoresDeInvolucrado(InvolucradoDTO loVictima) {
		...
		//Enable JC. Solucion al ERROR 8
		List<DetencionDTO> detenciones = loVictima.getDetenciones();
		if(detenciones != null)
			for(DetencionDTO det : detenciones)	det.setDetencionId(null);
	}
	...
	public ExpedienteDTO generarNuevoExpedienteConCaso(ExpedienteDTO expedienteDTO) throws NSJPNegocioException{
		...
		/************************************fin del else ***********************/
		if(expedienteDTO.getNumeroExpediente() !=null && expedienteDTO.getCasoDTO().getNumeroGeneralCaso()!= null){

			
			logger.debug("Action proviene desde registrar sentencia................");
			
			CasoDTO casoDTO = new CasoDTO();
			casoDTO.setNumeroGeneralCaso(expedienteDTO.getCasoDTO().getNumeroGeneralCaso());
			
			/*retorna el caso creado*/
			Date fechaApertura = casoDTO.getFechaApertura()==null?new Date():casoDTO.getFechaApertura();
			casoDTO.setFechaApertura(fechaApertura);
			Long idcaso=casoDAO.create(CasoTransformer.transformarCasoBasico(casoDTO));
			//casoDTO= CasoTransformer.transformarCasoBasico(casoDAO.consultarCasoPorNumeroCaso(casoDTO.getNumeroGeneralCaso()));
			
			logger.info("Caso id creado:: "+casoDTO.getCasoId());
			
			/*Creando el expediente*/
			ExpedienteDTO expnuevo=new ExpedienteDTO();
			/*setenado el caso id al expediente*/
			expnuevo.setCasoDTO(CasoTransformer.transformarCasoBasico(casoDAO.read(idcaso)));
			expnuevo.setFechaApertura(fechaApertura);
			expnuevo.setNumeroExpediente(expedienteDTO.getNumeroExpediente());
			expnuevo.setArea(expedienteDTO.getUsuario().getAreaActual());
			expnuevo.setUsuario(expedienteDTO.getUsuario());
			expnuevo.setEstatus(new ValorDTO(EstatusExpediente.ABIERTO.getValorId()));
			ConfInstitucion ConfInstitucion = confInsDao.consultarInsitucionActual();
			expnuevo.setPertenceConfInst(ConfInstitucionTransformer.transformarInstitucion(ConfInstitucion));
			expnuevo.setDiscriminante(expedienteDTO.getUsuario().getFuncionario().getDiscriminante());
			
	       Long idExp= expedienteDAO.create(ExpedienteTransformer.transformarExpediente(expnuevo));
	       
	       expnuevo.setExpedienteId(idExp);

	       NumeroExpediente noExpBD = new NumeroExpediente();
	       
	       noExpBD.setNumeroExpediente(expedienteDTO.getNumeroExpediente());
	       noExpBD.setFechaApertura(fechaApertura);
           noExpBD.setExpediente(new Expediente( expnuevo.getExpedienteId()));
           ...
           noExpBD.setNumExpAlterno(expedienteDTO.getNumeroExpediente());

           Long idNoExpNuevo = this.numeroExpedienteDAO.create(noExpBD);

           logger.debug("El id del Numero de expediente creado es :: "+idNoExpNuevo);
           
           noExpBD.setNumeroExpedienteId(idNoExpNuevo);
       
           expnuevo.setNumeroExpedienteId(idNoExpNuevo);
	       
           logger.info("ExpedienteDTO ::"+expnuevo);
...
/***********************************************************************************/
			...
			/*********************************tambien proviene de sentencia pero consultar a partir de caso, ya existe en sigi 
			 * solo hay que crear el numero de expediente ya que se generara uno por cada involucrado 
			 *  
			 * ***fin del else ***********************/
			ExpedienteDTO expBD = null;
			CasoDTO casoDTO = new CasoDTO();
		...
		if(expedienteDTO.getCasoDTO()!=null && expedienteDTO.getCasoDTO().getNumeroGeneralCaso()!=null){
			...
			Long idCaso=casoDAO.consultarIdXNumeroCaso(expedienteDTO.getCasoDTO().getNumeroGeneralCaso());
			
			if(idCaso > 0 && idCaso != null ){
				
				casoDTO=CasoTransformer.transformarCasoBasico(casoDAO.read(idCaso));
				
				List<ExpedienteDTO> expedientes = ExpedienteTransformer.transformarExpediente(expedienteDAO.consultarExpedientesPorIdCaso(casoDTO));
				
				for (ExpedienteDTO expedienteDTO2 : expedientes) {
					
					if(expedienteDTO2.getCasoDTO().getCasoId().equals(idCaso)){
						UsuarioDTO usuarioDTO=expedienteDTO.getUsuario();
						expedienteDTO2.setUsuario(usuarioDTO);
						expedienteDTO2.setArea(expedienteDTO.getArea());
						expBD=asignarNumeroExpedienteService.asignarNumeroExpediente(expedienteDTO2);
						break;
					}
