...
public class ConsultarConvenioServiceImpl implements
	 ...
	 */
	@Override
	public List<ConvenioDTO> consultarConvenioTipoGuardado(
			List<ConvenioDTO> convenios) throws NSJPNegocioException {
		if (logger.isDebugEnabled())
			logger.debug("/**** SERVICIO PARA CONSULTAR LOS ACUERDOS RESTAURATIVOS RELACIONADOS A UN NUMERO DE EXPEDIENTE ****/");
		...
		for(ConvenioDTO convenio: convenios ){
			Documento documento	=  documentoDAO.consultarDocumentoPorId(convenio.getConvenioID());
			if ( documento.getEsGuardadoParcial()== false ){
				convenio.setEsParcial(0L);
			}
			else{
				convenio.setEsParcial(1L);
			}
			acuerdosDTO.add(convenio);
		}
		return acuerdosDTO;
	}
