...
public class CatalogoTransformer {
	...
	public static List<CatalogoDTO> transformarDelitoSituacion(
			List<SituacionImputado> delitoSituacion) {

		final List<CatalogoDTO> resp = new ArrayList<CatalogoDTO>();
        for (SituacionImputado row : delitoSituacion) {
            resp.add(new CatalogoDTO(row.getSituacionImputadoId(), row
                    .getDescripcion()));
        }
		return resp;
	}
