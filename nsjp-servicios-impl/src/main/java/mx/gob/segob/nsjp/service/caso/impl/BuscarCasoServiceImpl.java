...
public class BuscarCasoServiceImpl implements BuscarCasoService {
    ...
    @Override
  	public List<CasoDTO> consultarCasos() throws NSJPNegocioException {

      	List<Caso> listaCasos = new ArrayList<Caso>();
      	listaCasos=casoDAO.findAll("casoId", true);
      	List<CasoDTO> listaCasoDTO = new ArrayList<CasoDTO>();
      	for (Caso caso : listaCasos) {
      		/**/
      		Caso casotemp = new Caso();
      		casotemp.setCasoId(caso.getCasoId());
      		casotemp.setNumeroGeneralCaso(caso.getNumeroGeneralCaso());
      		
  			listaCasoDTO.add(CasoTransformer.transformarCasoBasico(casotemp));
  		}
