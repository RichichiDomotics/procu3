...
public class PJClienteServiceImpl implements PJClienteService {
    ...
    public SolicitudAudienciaDTO enviarSolicitudAudiencia(
        ...
        try {
            System.out.print(confInsDao.read(Instituciones.PJ.getValorId())
                    .getUrlInst()
            ...
            if(expediente!=null &&
                    expediente.getUsuario()!=null &&
                    expediente.getUsuario().getFuncionario()!=null){
                if(expediente.getUsuario().getFuncionario().getPuesto()!=null &&
                        expediente.getUsuario().getFuncionario().getPuesto().getValor()!=null){
                    funSol.setNombrePuesto(expediente.getUsuario().getFuncionario().getPuesto().getValor());
                }
                if(expediente.getUsuario().getFuncionario().getJerarquiaOrganizacional()!=null &&
                        expediente.getUsuario().getFuncionario().getJerarquiaOrganizacional().getNombre()!=null){
                    funSol.setNombreArea(expediente.getUsuario().getFuncionario().getJerarquiaOrganizacional().getNombre());
                }
                funSol.setClaveFuncionario(expediente.getUsuario().getFuncionario().getClaveFuncionario());
                if(funSol.getClaveFuncionario()!=null){
                    List<CorreoElectronico> correoElects = correoElectronicoDAO.consultarCorreosByFuncionario(funSol.getClaveFuncionario());
                    if(correoElects!=null && !correoElects.isEmpty() && correoElects.get(0)!=null && correoElects.get(0).getDireccionElectronica()!=null){
                        funSol.setEmail(correoElects.get(0).getDireccionElectronica());
                    }
                }
            }
            ...
            //eNABLE IT INICIA BLOQUE By Asdrubal

            if(input.getIdsDocumentos()!=null && expediente.getDocumentosDTO()!=null){
                List<mx.gob.segob.nsjp.ws.cliente.solicitudaudiencia.DocumentoWSDTO> docsWS = new ArrayList<mx.gob.segob.nsjp.ws.cliente.solicitudaudiencia.DocumentoWSDTO>();

                String[] idDoc = input.getIdsDocumentos().split(",");

                for(DocumentoDTO doc : expediente.getDocumentosDTO()){

                    for(int i=0; i < idDoc.length; i++){
                        logger.info("Id de los Documentos input :::: " + idDoc[i] + "ID Docs Expediente :::: " + doc.getDocumentoId());
                        if( idDoc[i].equals(doc.getDocumentoId().toString() ) ){
                            doc.setFormaDTO(new FormaDTO(Formas.PLANTILLA_EN_BLANCO.getValorId())); // forma ID de documento en Blanco by Dorian
                            logger.info("ID Forma Modificadoooo  :::: " + doc.getFormaDTO().getFormaId());
                            docsWS.add( ObjetosWSDTOTransformer.transforma(doc));
//        				   docsWS.add(DocumentoWSDTOTransformer.transformarWSDTO(doc));

                        }
                    }
                }
                logger.info("HAY DOCUMENTOSWSDTO <<<"+ docsWS.size());
                toSend.getDocumentosDTO().addAll(docsWS);
            }
