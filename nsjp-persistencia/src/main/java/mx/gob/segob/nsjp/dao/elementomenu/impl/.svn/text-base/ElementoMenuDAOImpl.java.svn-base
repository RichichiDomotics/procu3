...
public class ElementoMenuDAOImpl extends
	...
	public List<ElementoMenu> consultarElementosMenuXRol(Rol rol, TipoMenu tm) {
		...
		if (tm != null) {
			...
			.append(" SELECT em.elementoMenuId FROM ElementoMenu em left join em.roles r WHERE r.rolId IN ( ");
			if( tm.getId() == 1 && rol.getNombreRol().equals("coordinadorAmp") ){
				queryString.append(rol.getRolId()).append(" )  )")
				.append(" AND em.elementoMenuId != 53 ");
			}
			else{
				queryString.append(rol.getRolId()).append(" ) ) ");
			}
			queryString.append(" ORDER BY em.iOrden");
