...
<!--	<jsp:include page="/WEB-INF/paginas/desarrolloJAVS/funcionesComunesJAVS.jsp" flush="true"></jsp:include>-->
	...
	*/
	...
	function configurarTabsJuez() {
	


		<%
			if(Roles.getByValor(rolDTO.getRolId()) != null) {
		%>
				//ocultando la opcion Resolutivos de Audiencia
				$("#resolutivos").hide();
				//mostrando la opcion Notas
				$("#notas").show();
				//Ocultando el editor
				$("#divNotasJuez").hide();
				//consultar notas
	...
	//carga nota
	function cargaNota(renglon){
		$("#fcreacion").show();
		var id = renglon.find('idNota').text();
		var row=$('#rowNota_'+id);
			var fecha="";
		try{
			fecha = renglon.find("fechaCreacion").text();
			fecha = fecha.substring(0,10);
		} catch(e){
		}
		$(row).remove();
		$('#tablaNotasExpediente').append('<tr id="rowNota_'+id+'"><td class="noSub">&nbsp;&nbsp;&nbsp;<a id="consultarNota_'+id+'" onclick="notaExpediente('+id+')">' + renglon.find("nombreNota").text() + '</a></td><td><a id="consultarNota_'+id+'" onclick="notaExpediente('+id+')">'+fecha+'</a></td></tr>');
		//cerrarVentanaNota();
	} 
	...
	*/
	...
	function consultarNotas(){
		...
		var notas=$('#notasJuez').val();
		$.ajax({
			type: 'POST',
			url: '<%= request.getContextPath()%>/consultarNotasExpediente.do?idNumeroExpediente='+idNumeroExpediente+'&porFuncionario=true',
			data: '',
			dataType: 'xml',
			async: false,
			success: function(xml){
...
//	 		EnableIT By Asdrubal se agregan variables para recuperar el nombre de la sala seleccionada
			...
			/*Enable IT :
				Eliminando validacion para poder agendar a cualquier hora sin tomar en cuenta la hora del servidor
			*/
	...
	*/
	function guardarAudiencia(){
		...
		//eNABLE IT -INICIO- By Asdrubal - Se agregan parametros para la solicitud de  audiencia
...
//**********************************************************************FUNCIONALIDAD DESCONOCIDA****************************************************************
   ...
   function calculaPena(){
    ...
    /* Funcion que permite acutalizar el caracter de una Audiencia
	*  Pública o Privada
	*/
	function actualizarCaracterAudiencia()
	{
			var parametros="";
			parametros+="idAudiencia="+idEvento;
			parametros+="&esPublica=" + $("#caracterPJENS").val();
			
			//mandamos a llamar al action que actualiza el caracter de una audiencia
			$.ajax({
				async: false,
				type: 'POST',
				url: '<%= request.getContextPath()%>/actualizarCaracterAudiencia.do',
				data: parametros,
				dataType: 'xml',
				success: function(xml){						
						...
						if(parseInt($(xml).find('code').text())==0)
						{	
							var bandera=$(xml).find('respuesta').text();
							if(bandera=="true")
							{
								customAlert("Se actualizó de manera correcta el carácter de la audiencia");
								parent.cargaGridPorFechaPJENS();
								parent.recargaGridAudienciasDelDia();								
							}
							else
							{
								customAlert("Ocurrió un error al actualizar el carácter de la audiencia");
							}							
						}
						else
						{
...
<div id="tabsDetalleAudiencias">
	...
	<ul>
    	...
    	<li id="notificaciones">
			<a href="#tabNotificaciones-11">Notificaciones</a>
		</li>
					...
					<tr>
		 			...
		 			<tr>
					    ...
					    <td colspan="3" align="center">
							<input type="button" id="btnGuardarAudiencia" class="btn_guardar" value="Guardar"  onclick="validaGuardarAudiencia();"/>
					    </td>
...
<div id="tabsDetalleAudiencias-9" >
				...
				<table width="100%">
						...
						<td>
							<table width="25%" cellpadding="0" cellspacing="0"
								...
								<tr>
									<td colspan="2">
										&nbsp;&nbsp;&nbsp;
										<input type="button" class="btn_modificar"
											value="Generar Nota" id="botonGuardarNotas"
											onclick="notaExpediente(0);" class="btn_Generico" />
									</td>
								</tr>
								<tr>
									<td>
									...
									<td>
									<span id="fcreacion" style="display: none;">
										Fecha De Creación
									</span>
									</td>
								</tr>								
					...
					<tr>
						<td>
							<div id="divNotasJuez">
							<textarea class="jquery_ckeditor" name="notasJuez" cols="70"
								rows="20" id="notasJuez"></textarea>
							</div>
						</td>
					</tr>
					<tr>
    ...
    <div id="tabNotificaciones-11" class="notificaciones">
...
<div  id="divAgregarResolutivo" style="display: none">
	<table width="500" border="0">
	  ...
	  <tr>
	    <td>&nbsp;</td>
	    <td align="center">
			<input name="text" type="text" id="tempVideo" style="width:150px; border: 0; background:#DDD;" readonly="readonly"/>
		</td>
	    <td>&nbsp;</td>
	  </tr>
	  <tr>
