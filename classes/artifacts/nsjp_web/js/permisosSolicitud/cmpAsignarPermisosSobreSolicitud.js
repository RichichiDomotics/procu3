	...
	if (rowd) {
		...
		if(gridAPSEGenerado){
			...
			if(areaActualFunc == 12){
				headers = ['Solicitud', 'Tiempo de Asignacion', 'Lectura', 'Escritura'];
			}else{
				headers = ['Expediente', 'Tiempo de Asignacion', 'Lectura', 'Escritura'];
			}
...
function asignarPermisoSobreSolicitudes(){
	...
	if(!isEmpty(idSubordinado) && !isEmpty(idSolicitud)){
		...
		if(!isEmpty($("#fechaVencimiento").val())){
			...
			alertDinamico("Debe de seleccionar una Tiempo de Asignacion para el permiso.");
		}
	}else{
